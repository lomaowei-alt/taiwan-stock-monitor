...
"""
台灣股市月度績效監測 - 資料抓取腳本
每天執行，從 TWSE / TPEx 官方 API 抓取資料，生成靜態 HTML
"""
...
HEADERS = {
    ...
    "User-Agent": "Mozilla/5.0 (compatible; TaiwanStockMonitor/1.0)"
}
...
def main():
    ...
    # ── 建立 lookup maps ─────────────────────────────────
    twse_ind = {
        (r.get("公司代號") or r.get("股票代號") or "").strip():
        (r.get("產業類別") or "").strip()
        for r in twse_info
    }
    tpex_ind = {
        (r.get("SecuritiesCompanyCode") or r.get("公司代號") or "").strip():
        (r.get("IndustryCode") or r.get("IndustryName") or r.get("產業別") or "").strip()
        for r in tpex_info
    }
    twse_fin = {}
    ...
    for r in twse_rev:
        ...
        if not rev or rev <= 0:
            continue
        yoy = (float(r["當月營收年增率"]) if r.get("當月營收年增率") not in (None,"","N/A")
               else calc_pct(rev, pn(r.get("去年同月營收"))))
        mom = (float(r["當月營收月增率"]) if r.get("當月營收月增率") not in (None,"","N/A")
               else calc_pct(rev, pn(r.get("上月營收"))))
    ...
    print(f"✓ 已輸出 docs/data.json ({size_kb:.0f} KB)")
